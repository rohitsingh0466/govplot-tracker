...
const PLANS = [
  ...
  },
  {
    ...
    tag: "Most Popular",
    features: [
      { ok: true, text: "All 9 cities tracked" },
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="GovPlot Tracker plans: Free, Pro ₹99/mo, Premium ₹299/mo. Track government plot schemes with real-time alerts." />
      ...
      <div className="page-container pt-12 pb-20">
        ...
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <p className="text-[16px] text-[--ink-600] leading-relaxed">
            Start free. Upgrade when you need more cities, channels, or notifications.
          </p>
          ...
          <div className="inline-flex items-center gap-3 mt-6 bg-[--ink-50] border border-[--ink-100] rounded-full px-2 py-1.5">
            <button
              ...
              className={`px-5 py-1.5 rounded-full text-[13px] font-semibold transition ${!annual ? "bg-white text-[--ink-900] shadow-sm" : "text-[--ink-500]"}`}
            >
              Monthly
            </button>
            <button
              onClick={() => setAnnual(true)}
              className={`px-5 py-1.5 rounded-full text-[13px] font-semibold transition flex items-center gap-2 ${annual ? "bg-white text-[--ink-900] shadow-sm" : "text-[--ink-500]"}`}
            >
              Annual
            ...
            return (
              <div
                ...
                className={`relative rounded-3xl border-2 ${plan.border} overflow-hidden ${
                  plan.id === "pro" ? "md:-mt-4 md:mb-4 shadow-2xl" : "shadow-sm"
                }`}
              >
                {plan.tag && (
                ...
                <div className="bg-white p-6">
                    ...
                    {plan.features.map(({ ok, text }) => (
                      <li key={text} className="flex items-center gap-2.5 text-[13px]">
                        <span className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[11px] ${
                          ok ? "bg-[--teal-100] text-[--teal-700]" : "bg-[--ink-50] text-[--ink-300]"
                        }`}>
                          {ok ? "✓" : "—"}
                        </span>
                  ...
                  <Link href={plan.btnHref} className={`${plan.btn} w-full justify-center text-[14px] py-3`}>
                    {plan.btnLabel}
                  </Link>
                </div>
              </div>
        ...
        <div className="max-w-2xl mx-auto mt-20">
          ...
          <div className="space-y-4">
            {[
              ...
              { q: "What is the difference between Pro and Premium?", a: "Pro covers all 9 cities with all notification channels. Premium adds B2B API access, multi-user seats, custom authority alerts, and priority support." },
            ].map(({ q, a }) => (
