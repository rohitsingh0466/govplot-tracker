      ...
      <div className="min-h-screen bg-[--bg-page]">
        ...
        <div className="max-w-3xl mx-auto px-4 py-12">
          ...
          <div className="space-y-6">
            {POSTS.map((post, i) => (
              <article key={post.slug} className="card card-hover p-6 animate-fade-in-up" style={{ animationDelay: `${i * 60}ms` }}>
                <div className="flex flex-wrap items-center gap-2 mb-3">
                  <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full ${TAG_COLORS[post.tag] || TAG_COLORS.Guide}`}>{post.tag}</span>
                  {post.city !== "All" && (
                    <span className="text-[11px] font-semibold text-[--ink-500] bg-[--ink-50] border border-[--ink-100] px-2.5 py-1 rounded-full">📍 {post.city}</span>
                  )}
