"""
GovPlot Tracker — AUDA Ahmedabad Scraper
Rank: 15 | Authority: Ahmedabad Urban Development Authority
...
class AUDAScraper(CityScraperMixin, BaseScraper):
    CITY = "Ahmedabad"
    AUTH = "AUDA"
    ...
    TIER1_URLS = [
        ...
        "https://auda.org.in/residential-plot",
    ]
    AGGREGATOR_URLS = [
        "https://www.eauctionsindia.com/blog-details/ahmedabad",
        "https://awaszone.com/ahmedabad/",
    ]
    ...
    def _parse(self, soup, source_url: str) -> list[SchemeData]:
        ...
        for el in candidates:
            ...
            schemes.append(make_scheme(
                ...
                location_details="Ahmedabad Urban Development Authority zone",
            ))
        return schemes
    ...
    def scrape_aggregators(self) -> list[SchemeData]:
        ...
        for url in self.AGGREGATOR_URLS:
            ...
            parsed = self._parse_aggregator_generic(
                soup, url, self.AUTH, self.CITY, self.BASE_URL, "Ahmedabad Urban Development Authority zone"
            )
