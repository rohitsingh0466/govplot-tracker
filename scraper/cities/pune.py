"""
GovPlot Tracker — PMRDA / MHADA Pune Scraper
=============================================
Rank: 9 | Demand: HIGH | Tier: 2 (Playwright)
Authorities: PMRDA + MHADA-PUNE (secondary)
Known schemes: PMRDA Housing Lottery 833 units, MHADA Pune 6294 units
"""
...
class PMRDAScraper(CityScraperMixin, BaseScraper):
    ...
    def scrape_tier1(self):
        for url in self.TIER1_URLS:
            ...
            if soup:
                s = self._parse(soup, url, "PMRDA" if "pmrda" in url else "MHADA-PUNE")
                if s: return s
        return []
    def scrape_tier2(self):
        for url in self.TIER1_URLS[:2]:
            soup = self.get_playwright_soup(url, wait_selector="table, .scheme-list, main", wait_secs=12)
            if soup:
                s = self._parse(soup, url, "PMRDA")
                if s: return s
        return []
    def _parse(self, soup, source_url, authority="PMRDA"):
        schemes = []
        for el in (soup.select("table tr, div.scheme, article, li") or []):
            text = el.get_text(separator=" ", strip=True)
            if not any(k in text.lower() for k in ("plot", "residential", "lottery", "scheme")): continue
            if any(k in text.lower() for k in ("lig", "ews", "e-auction", "commercial")): continue
            if len(text) < 15: continue
            name_el = el.select_one("h2, h3, h4, td, strong, a")
            raw = name_el.get_text(strip=True) if name_el else text[:120]
            if len(raw) < 10: continue
            name = raw if raw.startswith(authority) else f"{authority} {raw}"
            ...
            schemes.append(make_scheme(authority, self.CITY, name, self.normalise_status(text), source_url, data_source="LIVE", apply_url=apply_url, total_plots=self.parse_plots(text), location_details="Pune Metropolitan Region"))
        return schemes
    def scrape_aggregators(self):
        s = []
        for url in self.AGGREGATOR_URLS:
            soup = self.get_soup(url)
            s.extend(self._parse_aggregator_generic(soup, url, self.AUTH, self.CITY, self.BASE_URL, "Pune"))
        return s
