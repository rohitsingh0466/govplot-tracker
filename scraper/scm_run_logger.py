...
class AuthorityRunResult:
    ...
    Passed to SCMRunLogger.record_authority_result().
    """
    authority_code:     str
    authority_id:       Optional[str]       = None   # UUID from scraper_authorities
    city:               str                 = ""
    url_config_id:      Optional[str]       = None   # UUID of URL that succeeded
    url_attempted:      Optional[str]       = None   # actual URL tried
    url_type:           Optional[str]       = None   # primary | alternative | etc.
...
class SCMRunLogger:
    ...
    def __init__(self, run_mode: str = "full", scraper_version: str = "v4.0"):
    ...
    def flush(
        ...
        Non-blocking — any error is logged and swallowed so scraper doesn't crash.
        """
        if not self._enabled:
            return
        ...
        with httpx.Client(timeout=20) as client:
            ...
            # For each result that has a url_config_id, update last_success_at
            # or increment failure_count + last_failure_at on the URL config row.
            for r in self.results:
                ...
                try:
                    if r.status == "ok":
                        ...
                        # Increment failure count via RPC (no read-modify-write race)
                        client.post(
